...
class WXR_Tests_Namespace_Aware_Import extends WXR_Import_UnitTestCase {
	...
	function test_small_import() {
		...
		// copy the export file because the import will transform it
		$file = '/data/export/small-export-wxr12.xml';
		copy( __DIR__ . $file, __DIR__ . "{$file}.tmp" );

		$authors = array( 'admin' => false, 'editor' => false, 'author' => false );
		$this->_import_wp( __DIR__ . $file, $authors );

		// restore the original version, for future unit test runs
		rename( __DIR__ . "{$file}.tmp", __DIR__ . $file );
	...
	function test_small_import_ns() {
		...
		// copy the export file because the import will transform it
		$file = '/data/export/small-export-wxr12-ns-aware.xml';
		copy( __DIR__ . $file, __DIR__ . "{$file}.tmp" );
	 ...
	 * in the WXR instance.
	 *
	 * @todo once this plugin has the necessary hooks, add assertions to check
	 * that the extension markup got imported correctly.
	 */
	function test_small_import_native_wxr13() {
		global $wpdb;

		$authors = array( 'admin' => false, 'editor' => false, 'author' => false );
		$this->_import_wp( __DIR__ . '/data/export/small-export-native-wxr13.xml', $authors );
